...
function BadgeCustomizerTab({ user }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 🎯 SISTEMA CONSOLIDADO: Categorías + Presets + Overrides en UNA sola vista */}
...
// ============================================
// COMPONENTE: CategorySection (CONSOLIDADO)
// ============================================
...
function CategorySection({
  ...
  const currentCategoryPreset = presetConfig.categories?.[categoryKey] || presetConfig.defaultPreset || 'full';
        ...
        <div className="p-4 space-y-3">
          {Object.entries(badges).map(([badgeKey, badge]) => (
            <BadgeRow
              ...
              presetConfig={presetConfig}
              displayPresets={displayPresets}
              onUpdateBadgePreset={onUpdateBadgePreset}
              onRemoveBadgePreset={onRemoveBadgePreset}
            />
          ))}
        </div>
      )}
...
// ============================================
// COMPONENTE: BadgeRow (CONSOLIDADO)
// ============================================
...
function BadgeRow({
  ...
  const categoryDefaultPreset = presetConfig.categories?.[categoryKey] || 'full';
  ...
  return (
    <div
      ...
      style={{
        background: 'var(--color-bg-primary)',
        border: hasOverride ? '2px solid var(--color-primary)' : '1px solid var(--color-border)',
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {/* Color picker + Preset selector */}
        <div className="flex items-center gap-2">
          {/* 🆕 Selector de Preset Individual */}
          <select
            value={currentPreset}
            onChange={(e) => {
              const value = e.target.value;
              if (value === categoryDefaultPreset) {
            ...
            title="Preset de visualización individual"
          >
            <option value={categoryDefaultPreset}>
              📂 Categ.
            </option>
            {Object.entries(allPresets).map(([key, preset]) => (
              <option key={key} value={key}>
