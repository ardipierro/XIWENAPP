...
/**
 * Hook para gestionar la configuración del Design Lab
 ...
 * @example
 * function DesignLab() {
 *   const { config, loading, updateConfig, resetConfig } = useDesignLabConfig();
 *
 *   return (
 ...
 * }
 */
export function useDesignLabConfig() {
  const { user } = useAuth();
  ...
  const loadConfig = useCallback(async () => {
    try {
      ...
      if (user?.uid) {
        const userConfig = await getDesignLabConfig(user.uid);
        setConfig(userConfig);
        logger.debug('Design Lab config loaded');
      } else {
      ...
      logger.error('Error loading Design Lab config:', err);
  ...
  const updateConfig = useCallback(
    async (updates) => {
      try {
        ...
        if (user?.uid) {
          await saveDesignLabConfig(user.uid, newConfig);
          logger.debug('Design Lab config saved');
        } else {
  ...
  const updateField = useCallback(
    async (field, value) => {
      try {
        ...
        if (user?.uid) {
          await updateDesignLabConfigField(user.uid, field, value);
          logger.debug(`Design Lab config field '${field}' updated`);
        } else {
  ...
  const resetConfig = useCallback(async () => {
    try {
      ...
      if (user?.uid) {
        await resetDesignLabConfig(user.uid);
...
export default useDesignLabConfig;
